...
from .models import VoterRegistrationModel,PartiesRegistrationModel
...
class VoterRegistrationForm(forms.ModelForm):
    ...
    aadhar = forms.CharField(widget=forms.TextInput(
        attrs={'autocomplete': 'off', 'pattern': '[123456789][0-9]{11}', 'title': 'Enter Characters Only '}), required=True,
        max_length=100)
    age = forms.CharField(widget=forms.TextInput(attrs={'pattern': '[123456789][0-9]{1}'}), required=True,
                             max_length=100)
...
class PartiesRegistrationForm(forms.ModelForm):
    name = forms.CharField(widget=forms.TextInput(attrs={'pattern': '[a-zA-Z]+'}), required=True, max_length=100)
    mobile = forms.CharField(widget=forms.TextInput(attrs={'pattern': '[56789][0-9]{9}'}), required=True,
                             max_length=100)
    email = forms.CharField(widget=forms.TextInput(attrs={'pattern': '[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$'}),
    ...
    locality = forms.CharField(widget=forms.TextInput(), required=True, max_length=100)
    address = forms.CharField(widget=forms.Textarea(attrs={'rows': 4, 'cols': 22}), required=True, max_length=250)
    partysymbol = forms.FileField()
    partyname = forms.CharField(widget=forms.TextInput(
        attrs={'autocomplete': 'off', 'pattern': '[A-Za-z ]+', 'title': 'Enter Characters Only '}), required=True,
        max_length=100)
    status = forms.CharField(widget=forms.HiddenInput(), initial='activated', max_length=100)

    class Meta():
        model = PartiesRegistrationModel
        fields = '__all__'
